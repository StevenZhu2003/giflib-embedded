...
The following values are generated by the calculator with its verified ARM32 defaults, BURST_READ disabled, 256-entry global and local palettes per live decoder, and a 64-byte port handle. They are only an order-of-magnitude aid; use the calculator for the actual product envelope.
