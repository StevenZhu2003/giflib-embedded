...
/* Dispatch all input through the user-supplied byte-source callback. */
static int InternalRead(GifFileType *gif, GifByteType *buf, int len) {
    GifFilePrivateType *Private = (GifFilePrivateType *)gif->Private;

    return Private->Read(gif, buf, len);
}
...
/******************************************************************************
 GifFileType constructor with user supplied input function
******************************************************************************/
GifFileType *DGifOpen(void *userData, InputFunc readFunc, int *Error) {
    ...
    GifFile = (GifFileType *)gif_mem_malloc(sizeof(GifFileType));
    if (GifFile == NULL) {
        if (Error != NULL) {
    ...
    Private = (GifFilePrivateType *)gif_mem_calloc(1, sizeof(GifFilePrivateType));
    if (Private == NULL) {
        if (Error != NULL) {
            *Error = D_GIF_ERR_NOT_ENOUGH_MEM;
        }
        gif_mem_free((char *)GifFile);
        return NULL;
    }
    ...
    if (DGifGetScreenDesc(GifFile) == GIF_ERROR) {
        /* Preserve the specific I/O or allocation failure reported while
         * reading the logical screen descriptor. */
        int ScreenError = GifFile->Error;

        GifFreeMapObject(GifFile->SColorMap);
        gif_mem_free((char *)Private);
        gif_mem_free((char *)GifFile);
        if (Error != NULL) {
            *Error = ScreenError != D_GIF_SUCCEEDED
                         ? ScreenError
...
int DGifGetImageHeader(GifFileType *GifFile) {
	...
	/* GifWord is signed, but values read from the data stream are unsigned
	 * 16-bit quantities.  Multiply in unsigned long after checking the
	 * target width so a 32-bit signed long cannot overflow. */
	if (GifFile->Image.Height != 0 &&
	    (unsigned long)GifFile->Image.Width >
	        ULONG_MAX / (unsigned long)GifFile->Image.Height) {
...
int DGifGetImageDesc(GifFileType *GifFile) {
	...
	if (GifFile->SavedImages) {
		SavedImage *new_saved_images = (SavedImage *)gif_mem_realloc_array(
		...
		if ((GifFile->SavedImages =
		         (SavedImage *)gif_mem_malloc(sizeof(SavedImage))) == NULL) {
			GifFile->Error = D_GIF_ERR_NOT_ENOUGH_MEM;
			return GIF_ERROR;
		}
	}
...
/******************************************************************************
This routine should be called last, to close the GIF decoder.
******************************************************************************/
int DGifCloseFile(GifFileType *GifFile, int *ErrorCode) {
	...
	if (!IS_READABLE(Private)) {
		...
		gif_mem_free((char *)GifFile->Private);
		gif_mem_free(GifFile);
		return GIF_ERROR;
	}
	...
	while (i < LineLen) { /* Decode LineLen items. */
		...
		if (CrntCode == EOFCode) {
			...
			if (CrntCode < ClearCode) {
				...
				Stack[StackPtr++] = (GifByteType)CrntPrefix;
			...
			if (LastCode != NO_SUCH_CODE &&
				...
				if (CrntCode == Private->RunningCode - 2) {
					...
					Suffix[Private->RunningCode - 2] =
					    (GifByteType)DGifGetPrefixChar(Prefix, LastCode,
					                      ClearCode);
				} else {
					Suffix[Private->RunningCode - 2] =
					    (GifByteType)DGifGetPrefixChar(Prefix, CrntCode,
					                      ClearCode);
				}
			}
			LastCode = CrntCode;
...
void DGifDecreaseImageCounter(GifFileType *GifFile) {
	...
	if (GifFile->SavedImages[GifFile->ImageCount].RasterBits != NULL) {
		gif_mem_free(GifFile->SavedImages[GifFile->ImageCount].RasterBits);
	}
	...
	if (GifFile->ImageCount <= 0) {
		gif_mem_free(GifFile->SavedImages);
		GifFile->SavedImages = NULL;
	...
	SavedImage *correct_saved_images = (SavedImage *)gif_mem_realloc_array(
